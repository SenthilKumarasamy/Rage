    ...
    for i in ListStreams:
        ...
        if (not isinstance(i,Energy_Stream)):
            ...
            if (InUint !=[] and OutUint ==[]):
                ...
                G.add_node(Str1)
                Dic={'Name':i.Name}
                G.add_edge(Str1,InUint,Dic)
                ListEdges.append([Str1,InUint,i.Name])
                ListStart.append(i)
            elif (InUint ==[] and OutUint != []):
                Str1='End'+str(len(ListEnd))
                G.add_node(Str1)
                Dic={'Name':i.Name}
                G.add_edge(OutUint,Str1,Dic)
                ListEdges.append([OutUint,Str1,i.Name])
                ListEnd.append(i)
            elif (InUint !=[] and OutUint !=[]):
                Dic={'Name':i.Name}
                G.add_edge(OutUint,InUint,Dic)
                ListEdges.append([OutUint,InUint,i.Name])
